...
const HeroPersonalTouch = () => {
  ...
  return (
    <div className={`flex flex-col sm:flex-row items-start justify-start gap-4 ${isMobile ? 'my-6 mb-8' : 'my-8 mb-12'} animate-on-scroll delay-700 ${isMobile ? 'max-w-full' : ''}`}>
      <div className={`bg-black/50 backdrop-blur-sm ${isMobile ? 'p-4 rounded-lg max-w-full' : 'p-4 md:p-5 lg:p-6 rounded-lg max-w-lg'} border border-white/30 shadow-lg w-full sm:w-auto flex flex-row items-center`}>
        <img 
          src="/lovable-uploads/069112d9-e61f-4def-94ed-7f1c34172bfd.png"
          alt="Jayden Fisher - Owner" 
          className={`${isMobile ? 'w-14 h-14' : 'w-16 h-16 md:w-18 md:h-18 lg:w-20 lg:h-20'} rounded-full border-2 border-white shadow-md mr-4 md:mr-5 flex-shrink-0`}
        />
