...
export const NavLink = ({ to, isOverVideo, children, className = '' }: NavLinkProps) => {
  ...
  return (
    <Link 
      to={to} 
      className={`relative font-bold transition-all duration-300 hover:scale-110 tracking-wide ${
        isOverVideo 
        ? 'text-white hover:text-bc-red text-shadow-lg drop-shadow-lg' 
        : `text-black hover:text-bc-red ${isActive ? 'font-bold text-bc-red' : ''}`
