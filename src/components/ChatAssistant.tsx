...
const ChatAssistant = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const faqSection = document.querySelector('h2')?.textContent?.includes('Frequently Asked Questions') ? document.querySelector('h2')?.closest('section') : null;
      const satisfactionSection = document.querySelector('section h2')?.textContent?.includes('Satisfaction') ? document.querySelector('section h2')?.closest('section') : null;
      if (faqSection) {
  ...
  return <div className="fixed bottom-6 right-6 z-[1000]">
      <Button onClick={() => setIsOpen(!isOpen)} className={`chat-button rounded-full p-6 shadow-lg ${isOpen ? 'bg-bc-red hover:bg-bc-red/90' : 'bg-navy hover:bg-navy/90'}`} size="lg">
        {isOpen ? <X className="h-8 w-8 text-white" /> : <div className="relative">
            <img alt="Chat Assistant" className="h-12 w-12 rounded-full border-2 border-white" src="/lovable-uploads/970932d4-f8db-4a50-92b1-3fe35f2d06a7.jpg" />
      ...
      {!isOpen && <div className="absolute bottom-24 right-0 bg-white rounded-lg shadow-lg p-3 max-w-[250px] animate-fade-in">
          <p className="text-sm">{chatMessages[messageIndex]}</p>
        </div>}

      {isOpen && <div className="absolute bottom-24 right-0 w-[350px] bg-white rounded-xl shadow-xl p-5 border border-gray-200">
          <div className="flex items-center gap-4 mb-5">
            <img src="/lovable-uploads/2159ece1-41e3-4490-934d-771a1e1a10e0.png" alt="BC Pressure Washing Assistant" className="w-20 h-20 rounded-full object-cover border-2 border-bc-red" />
            <div>
          ...
          <div className="space-y-3 mb-5">
            ...
            <div className="bg-gray-100 rounded-lg p-3 hover:bg-gray-200 transition-all cursor-pointer animate-pulse" style={{
          animationDelay: "0.5s"
        }}>
