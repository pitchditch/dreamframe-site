...
const buildBalancedRouteBatches = (ranked: StorefrontCandidate[]) => {
  ...
  while (remaining.length > 0) {
    const batch: StorefrontCandidate[] = [];
    const counts = Object.fromEntries(Object.keys(perBatchCaps).map((key) => [key, 0])) as Record<StorefrontCategory, number>;

    for (let index = 0; index < remaining.length && batch.length < ROUTE_BATCH_SIZE;) {
      const candidate = remaining[index];
      if (counts[candidate.category] < perBatchCaps[candidate.category]) {
...
const processStorefrontElements = (
  ...
  for (const element of elements) {
    ...
    const address = streetAddress(tags) || name;
    ...
    const dedupeKey = phone
      ? `phone:${phone}|${normalizedAddress || normalizedName}`
      : websiteHost
        ? `web:${websiteHost}|${normalizedAddress || normalizedName}`
        : normalizedAddress
          ? `name-address:${normalizedName}|${normalizedAddress}`
          : `name-geo:${normalizedName}|${center.lat.toFixed(4)}|${center.lng.toFixed(4)}`;
...
const MapComponentV3: React.FC<MapComponentProps> = ({
  pins,
  routes,
  onAddPin,
  onClearAllPins,
  ...
  const saveCrawlSnapshot = useCallback((snapshot: CrawlSnapshot) => {
    const localSnapshot: CrawlSnapshot = {
      ...snapshot,
      id: snapshot.id || `crawl-${Date.now()}`,
      createdAt: snapshot.createdAt || new Date().toISOString(),
    };

    setCrawlHistory((previous) => {
      const next = [localSnapshot, ...previous].slice(0, 10);
      try { localStorage.setItem(CRAWL_HISTORY_KEY, JSON.stringify(next)); } catch { /* Supabase is authoritative */ }
      return next;
    });

    void saveD2DCrawlSession(snapshot).catch((error) => console.error('Could not save crawl session to Supabase:', error));
  }, []);
  ...
  useEffect(() => {
    ...
    setRouteMessage(activeRouteKind === 'storefront' ? 'Calculating balanced storefront walking route...' : 'Calculating walking route...');
          ...
          <div className="mt-2 pt-2 border-t space-y-2">
            ...
            {storefrontBatches.length > 1 && (
              <div className="flex flex-wrap gap-2">
                ...
                <span className="text-xs text-muted-foreground self-center">Batch {storefrontBatchIndex + 1}/{storefrontBatches.length} · max 8 food per 20</span>
              </div>
            )}
          </div>
        )}
