...
export interface OptimizedRoute {
  ...
  googleRouteDuration?: number;
  googleRouteDistance?: number;
  hasUphill?: boolean;
  hasDownhill?: boolean;
}
...
function extractCityFromAddress(address: string): string {
  ...
  for (const city of cities) {
    if (upperAddress.includes(city.toUpperCase())) return city;
  }
  return 'Unknown';
}
...
export function generateOptimizedRoutes(pins: HousePin[]): OptimizedRoute[] {
  ...
  cityGroups.forEach((cityPins, cityName) => {
    if (cityPins.length <= 1) return;
    const optimizedPins = optimizeRouteOrder(cityPins);
    routes.push({
      cityName,
      pins: optimizedPins,
      totalDistance: calculateRouteDistance(optimizedPins),
