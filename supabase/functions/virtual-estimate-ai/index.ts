...
const pricingInputKeys = [
  ...
  'access_notes',
  'condition_notes',
  'service_date_preference',
  'customer_mentioned_price',
] as const;
  ...
  properties: {
    ...
    pricing_inputs: {
      ...
      properties: Object.fromEntries(
        pricingInputKeys.map((key) => [key, { type: ['string', 'number', 'boolean', 'null'] }]),
      ),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (session.invite_expires_at && new Date(session.invite_expires_at).getTime() < Date.now()) {
    ...
    if (action === 'enable') {
      ...
      const { error } = await supabase
        .from('virtual_estimate_sessions')
        .update({ ai_assistant_enabled: true, ai_consent_at: consentAt, ai_last_updated_at: consentAt })
        .eq('id', session.id);
    ...
    if (action === 'disable') {
      const { error } = await supabase
        .from('virtual_estimate_sessions')
        .update({ ai_assistant_enabled: false, ai_last_updated_at: new Date().toISOString() })
        .eq('id', session.id);
      if (error) throw error;
    ...
    if (action === 'state') {
      const { data: events, error } = await supabase
        .from('virtual_estimate_transcripts')
        ...
        .eq('virtual_estimate_session_id', session.id)
        .order('sequence_number', { ascending: false })
        .limit(8);
      if (error) throw error;
      return respond({
        enabled: Boolean(session.ai_assistant_enabled),
        consentAt: session.ai_consent_at,
    ...
    if (!session.ai_assistant_enabled || !session.ai_consent_at) {
      return respond({ error: 'AI transcription has not been enabled by the customer.' }, 403);
    }

    const audio = typeof body?.audio === 'string' ? body.audio : '';
    const mimeType = typeof body?.mimeType === 'string' ? body.mimeType : 'audio/webm';
    const sequenceNumber = Number(body?.sequenceNumber);
    if (!audio || !Number.isInteger(sequenceNumber) || sequenceNumber < 1) {
      return respond({ error: 'audio and a positive sequenceNumber are required' }, 400);
    }
    ...
    formData.append('file', new Blob([bytes], { type: normalizedMime }), `virtual-estimate.${extensionForMime(normalizedMime)}`);
    formData.append('model', 'gpt-4o-mini-transcribe');
    formData.append(
      'prompt',
      'This is a BC Pressure Washing virtual estimate. Expect exterior cleaning terms such as window cleaning, panes, screens, tracks, gutters, downspouts, gutter guards, roof moss, soft wash, house wash, driveway, patio, deck, pressure washing, storefront glass, storeys, square feet, and service frequency.',
    );
    ...
    if (!transcriptionResponse.ok) {
      ...
      console.error('virtual-estimate-ai transcription failed', transcriptionResponse.status, detail);
    ...
    const analysisResponse = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        text: {
          format: {
            type: 'json_schema',
            name: 'virtual_estimate_analysis',
            strict: true,
    ...
    if (!analysisResponse.ok) {
      ...
      console.error('virtual-estimate-ai analysis failed', analysisResponse.status, detail);
    ...
    try {
      analysis = JSON.parse(rawAnalysis) as Analysis;
    } catch {
      console.error('virtual-estimate-ai invalid structured output', rawAnalysis);
      return respond({ error: 'Estimate analysis returned invalid structured data' }, 502);
    }
    ...
    const { data: inserted, error: insertError } = await supabase
      ...
      .upsert(
        {
          virtual_estimate_session_id: session.id,
    ...
    const { error: updateError } = await supabase
      .from('virtual_estimate_sessions')
      .update({ ai_summary: rollingSummary, ai_last_updated_at: now })
      .eq('id', session.id);
    if (updateError) throw updateError;

    return respond({ event: inserted, summary: rollingSummary });
  } catch (error) {
    console.error('virtual-estimate-ai error', error);
    return respond({ error: error instanceof Error ? error.message : 'Unknown server error' }, 500);
  }
});
